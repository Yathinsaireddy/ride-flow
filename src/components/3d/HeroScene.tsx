  ...
  return (
    <group ref={groupRef} scale={1.5}>
      ...
      <mesh position={[1.5, -0.5, 0]} rotation={[Math.PI / 2, 0, 0]}>
        ...
        <meshStandardMaterial color="#66FCF1" metalness={0.8} roughness={0.2} wireframe />
      </mesh>
      
      {/* Rear Wheel */}
      <mesh position={[-1.5, -0.5, 0]} rotation={[Math.PI / 2, 0, 0]}>
        <torusGeometry args={[0.5, 0.1, 16, 32]} />
        <meshStandardMaterial color="#66FCF1" metalness={0.8} roughness={0.2} />
      </mesh>
      
      {/* Frame (Abstract) */}
      <mesh position={[0, 0.2, 0]} rotation={[0, 0, Math.PI / 6]}>
        <boxGeometry args={[2.5, 0.15, 0.15]} />
        <meshStandardMaterial color="#C5C6C7" metalness={0.9} roughness={0.1} />
      </mesh>
      ...
      <mesh position={[1, 0.8, 0]}>
        ...
        <meshStandardMaterial color="#C5C6C7" metalness={0.9} />
      </mesh>
    </group>
  );
};
  ...
  return (
    <div className="absolute inset-0 w-full h-full -z-10 bg-gradient-to-br from-[#1a1a2e] to-[#16213e]">
      <Canvas camera={{ position: [0, 0, 6], fov: 50 }}>
        ...
        <spotLight position={[-10, -10, -5]} intensity={0.5} color="#45A29E" />
